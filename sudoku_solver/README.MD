...
```c++ std::vector<std::vector<int>> data = { {0, 6, 0, 0, 4, 5, 3, 7, 0}, {0, 0, 5, 6, 7, 3, 4, 2, 0}, {0, 0, 4, 0, 0, 0, 0, 0, 1}, {5, 0, 0, 7, 0, 2, 0, 0, 4}, {6, 0, 9, 0, 0, 0, 2, 5, 0}, {8, 0, 7, 0, 0, 9, 0, 0, 3}, {4, 9, 0, 5, 1, 7, 8, 0, 0}, {2, 1, 0, 0, 3, 6, 0, 0, 0},
{0, 5, 0, 0, 2, 0, 1, 0, 0}
};
```
...
```c++ bool process_solve(std::vector<std::vector<int>>& data, int row = 0, int cell = 0) { if (row == 9) return true;

 if (cell == 9) return process_solve(data, row + 1, 0);

 if (data[row][cell] != 0) return process_solve(data, row, cell + 1);

 for (int i = 1; i <= 9; ++i) { if (can_continue(data, row, cell, i)) { data[row][cell] = i;

 if (process_solve(data, row, cell + 1)) return true;
